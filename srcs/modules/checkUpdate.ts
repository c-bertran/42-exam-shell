...
export default async (): Promise<void> => {
	...
	return new Promise((resolve) => {
		get(
			'https://api.github.com/repos/c-bertran/codeshell/releases/latest',
			{
			...
			(res) => {
				...
				res.on('end', () => {
					...
					if (isUpdate(blob.tag_name)) {
						...
						console.log(`${format.format.reset}» ${format.foreground.light.green}https://github.com/c-bertran/codeshell/releases/latest${format.format.reset} «`);
						console.log(str2());
					}
					resolve();
				});
